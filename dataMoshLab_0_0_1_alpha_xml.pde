...
boolean dualDisplay, sort, filter, saveImg, text, prev, next, stroke, rasterize = false;
...
void draw() {
  ...
  */
  ...
  //size in pixels, x and y is for position, h and w height and width
    ...
    xText = imgOrigen.width/2;
    yText = imgOrigen.height/2;
    textAlpha = 255; //make this another slider?
    centerText = true; //if u just want the text cetered set this value to "true" and play with the positions referencing pxSize (half the photo size is a good starting point, so pxSize/2)
